...
nfw_mt = pd.read_csv(
    "extra_models/data/mt_nfw_list.csv", header=None, names=["t", "mt"]
)
nfw_mt["t"] = nfw_mt["t"].apply(eval)
nfw_mt_2 = nfw_mt.iloc[::-1]
nfw_mt_2["t"] = -1 * nfw_mt_2["t"]
nfw_mt_2 = nfw_mt_2.drop(0, axis=0)
nfw_mt_both = pd.concat([nfw_mt_2, nfw_mt], axis=0, ignore_index=True)

m_nfw_inner = make_interp_spline(nfw_mt_both["t"].values, nfw_mt_both["mt"].values)
...
dm_nfw_inner = m_nfw_inner.derivative()
...
def magnification_NFW(tau, beta, t_m, m, dm, return_impact_parameter=False):
...
class NFWmodel(PSPLmodel):
    def paczynski_model_parameters(self):
        """
        [t0, u0, tE, t_m]
        Boson star model parameters: standard PSPL + NFW mass parameter
        """
    ...
    def define_pyLIMA_standard_parameters(self):
        ...
        u0_bounds = (-1.0, 1.0)
        tE_bounds = (0.1, 500)
        tm_bounds = (0.1, 10.0)  # t_m bounds for NFW star parameter
        piEN_bounds = (-0.5, 0.5)
        piEE_bounds = (-0.5, 0.5)
    ...
    def model_magnification(
        ...
        if telescope.lightcurve is not None:
            ...
            source1_magnification = magnification_NFW(
                source1_trajectory_x,
                source1_trajectory_y,
            ...
            if source2_trajectory_x is not None:
                ...
                source2_magnification = magnification_NFW(
                    source2_trajectory_x,
                    source2_trajectory_y,
                    t_m=pyLIMA_parameters["t_m"],
                    m=m_nfw,
